...
import {I18nManager,TextInput,RefreshControl,Modal,PermissionsAndroid,Platform,ScrollView,StyleSheet,Text,ToastAndroid,View,Image,SafeAreaView,TouchableOpacity} from 'react-native';
...
export default class RechercheText extends Component {
	  ...
	  _Recherche=async () =>{
       ...
       /*
		...
		if(this.state.latitude==null || this.state.longitude==null){
			...
			alert(translate("recherche_text.enable_gps"))
		}else{
         */
			//this.showAlert()
			...
			bodyFormData.append('mot_cherche',this.searchedText)
			...
			bodyFormData.append('urlimage',null)
			...
			axios({
			method: 'post',
			url: `${Config.ApiBaseUrl}/recherche_text.php`,
			data: bodyFormData,
          ...
          <Content padder style={{marginBottom:20}}>
			...
			<View style={{ flexDirection:'row', flex: 1,alignContent:'center', alignItems:'center',justifyAlign:'center'}}>    
				<SafeAreaView style={{flex: 1}}>  
					<TextInput onSubmitEditing={()=>this._Recherche()} onChangeText={(text)=>this._searchTextInputChanged(text)} style={styles.textinput} placeholder={translate("recherche_text.search_product")}/>     
				</SafeAreaView> 
				...
				<TouchableOpacity
					...
					onPress={() => {
						if(this.searchedText.length>0){ 
							...
							alert(translate("recherche_text.field_empty"))
						}   
					}}
					>
					 <Icon active name={Platform.OS === "ios" ? "ios-search-circle-sharp" : "md-search-circle-sharp"} size={35} style={{marginRight:wp('3%'), marginLeft:wp("3%"), color:'green' }} /> 
				</TouchableOpacity>
            </View>
...

            {
              data.map((item, key) => (
                ...
                {item.urlimage=="" || this.state.affiche==false ?
                   <Text>{translate("recherche_text.product_not_found")}</Text>
                  :
                  <CardItem cardBody>
